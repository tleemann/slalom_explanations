...
class Embeddings(nn.Module):
    def __init__(self, config: PretrainedConfig):
        ...
        self.LayerNorm = lm.LayerNormEpsilon(config.dim, eps=1e-12)
    ...
    def forward(self, input_ids: torch.Tensor, input_embeds: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        embeddings = lf.add2(input_embeds, position_embeddings)  # (bs, max_seq_length, dim)
...
class MultiHeadSelfAttention(nn.Module):
    ...
    def forward(
        ...
        q = lf.mul2(q,  1.0 / math.sqrt(dim_per_head))  # (bs, n_heads, q_length, dim_per_head)
        scores = lf.matmul(q, k.transpose(2, 3))  # (bs, n_heads, q_length, k_length)
        mask = (mask == 0).view(mask_reshp).expand_as(scores)  # (bs, n_heads, q_length, k_length)
        scores = scores.masked_fill(
            mask, torch.tensor(torch.finfo(scores.dtype).min)
        )  # (bs, n_heads, q_length, k_length)

        weights = lf.softmax(scores, dim=-1)  # (bs, n_heads, q_length, k_length)
        weights = self.dropout(weights)  # (bs, n_heads, q_length, k_length)

        # Mask heads if we want to
        if head_mask is not None:
            weights = lf.mul2(weights, head_mask)

        context = lf.matmul(weights, v)  # (bs, n_heads, q_length, dim_per_head)
...
class TransformerBlock(nn.Module):
    def __init__(self, config: PretrainedConfig):
        ...
        self.sa_layer_norm = lm.LayerNormEpsilon(normalized_shape=config.dim, eps=1e-12)

        self.ffn = FFN(config)
        self.output_layer_norm = lm.LayerNormEpsilon(normalized_shape=config.dim, eps=1e-12)
    ...
    def forward(
        ...
        sa_output = self.sa_layer_norm(lf.add2(sa_output, x))  # (bs, seq_length, dim)

        # Feed Forward Network
        ffn_output = self.ffn(sa_output)  # (bs, seq_length, dim)
        ffn_output: torch.Tensor = self.output_layer_norm(lf.add2(ffn_output, sa_output))  # (bs, seq_length, dim)
...
class DistilBertForMaskedLM(DistilBertPreTrainedModel):
    ...
    def __init__(self, config: PretrainedConfig):
        ...
        self.vocab_layer_norm = lm.LayerNormEpsilon(config.dim, eps=1e-12)
