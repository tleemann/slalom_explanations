...
attnlrp = Composite({
    ...
    Conv1D: rules.EpsilonRule,
    GELUActivation: rules.IdentityRule,
    NewGELUActivation: rules.IdentityRule
})
...
def get_activation(activation_string):
    if activation_string == "gelu":
        ...
        raise ValueError(f"Other activation function {activation_string} currently not supported.")
...
class GPT2Attention(nn.Module):
    ...
    def _attn(self, query, key, value, attention_mask=None, head_mask=None):
        ...
        if self.scale_attn_weights:
            attn_weights = lf.mul2(attn_weights, torch.full([], value.size(-1) ** (-0.5), dtype=attn_weights.dtype, device=attn_weights.device))

        # Layer-wise attention scaling
        if self.scale_attn_by_inverse_layer_idx:
            attn_weights = lf.mul2(attn_weights, 1.0 / float(self.layer_idx + 1))
        ...
        attn_weights = lf.softmax(attn_weights, dim=-1)
    ...
    def _upcast_and_reordered_attn(self, query, key, value, attention_mask=None, head_mask=None):
        ...
        with torch.amp.autocast(query.device.type, enabled=False):
            ...
            attn_weights = lf.mul2(lf.matmul(q.float(), k.float()), scale_factor.to(device=query.device))
...
class GPT2SdpaAttention(GPT2Attention):
    ...
    def forward(
        ...
        print("output_attentions", output_attentions, "head_mask", head_mask)
        ...
        attn_output = scaled_dot_product_attention(query, key, value, 
            attn_mask=attention_mask, dropout_p=self.attn_dropout.p if self.training else 0.0, is_causal=is_causal,
        )
...
class GPT2Block(nn.Module):
    def __init__(self, config, layer_idx=None):
        ...
        self.ln_1 = lm.LayerNormEpsilon(hidden_size, eps=config.layer_norm_epsilon)
        self.attn = attention_class(config=config, layer_idx=layer_idx)
        self.ln_2 = lm.LayerNormEpsilon(hidden_size, eps=config.layer_norm_epsilon)

        if config.add_cross_attention:
            self.crossattention = attention_class(config=config, is_cross_attention=True, layer_idx=layer_idx)
            self.ln_cross_attn = lm.LayerNormEpsilon(hidden_size, eps=config.layer_norm_epsilon)
    ...
    def forward(
        ...
        # residual connection
        hidden_states = lf.add2(attn_output, residual)
        ...
        if encoder_hidden_states is not None:
            ...
            # residual connection
            hidden_states = lf.add2(residual, attn_outpu)
        ...
        # residual connection
        hidden_states = lf.add2(residual, feed_forward_hidden_states)
...
class GPT2PreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        elif isinstance(module, lm.LayerNormEpsilon):
            module.bias.data.zero_()
...
class GPT2Model(GPT2PreTrainedModel):
    def __init__(self, config):
        ...
        self.ln_f = lm.LayerNormEpsilon(self.embed_dim, eps=config.layer_norm_epsilon)
    ...
    def forward(
        ...
        hidden_states = lf.add2(inputs_embeds, position_embeds.detach())
        ...
        if token_type_ids is not None:
            ...
            hidden_states = lf.add2(hidden_states, token_type_embeds.detach())
...
class GPT2ForSequenceClassification(GPT2PreTrainedModel):
    ...
    def forward(
        ...
        """
        return_dict = return_dict if return_dict is not None else self.config.use_return_dict
        print("output attn passed to forward", output_attentions)
        transformer_outputs = self.transformer(
            input_ids,
            past_key_values=past_key_values,
            attention_mask=attention_mask,
