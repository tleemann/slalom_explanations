...
def slalom_scatter_plot(expl_list: tp.List[tp.Tuple[str, np.ndarray]], sizex=12, sizey=12, ylim=None, highlight_toks=[], fontsize=6):
    ...
    for tok_str, slalom_scores in expl_list:
        if tok_str in plotted_set:
            continue
        tok_str = tok_str.replace("Ġ", "").replace("##", "") ## For GPT2-tokens and subword tokens
        if tok_str in highlight_toks:
            plt.scatter([slalom_scores[0]], [slalom_scores[1]], 3, c="tab:red")
            plt.annotate(tok_str, xy=(slalom_scores[0], slalom_scores[1]), fontsize=fontsize)
        else:
...
def slalom_highlight_plot(expl_list: tp.List[tp.Tuple[str, np.ndarray]], max_len=None, start_pos_y=500, vmax=5):
